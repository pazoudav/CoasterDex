...
def extractFeatures(imgs, model, RMAC, L, resolutionLevel, bBox=[], croppedActivations = False):
	...
	for j in tqdm(range(0,len(imgs))):
		for i in range(0, resolutionLevel):
			...
			extractFeatures_(img, listData, i, deltas, model, croppedActivations, bBox, j, RMAC, L)
	return listData
	

def extractFeatures_(img, listData, i, deltas, model, croppedActivations, bBox, j, RMAC, L):
